...
<div class="main-content user-pro-dtls-page usr-feed-page-nw about-page sub-about">
...
//				$profileqry=mysqli_query($conn,"SELECT * FROM user_inf WHERE pet_unique_id='$profile_uid'");
			       $profileqry=mysqli_query($conn,"SELECT pet_name, profile_pic, bg_image, about_data, pet_unique_id FROM user_inf WHERE pet_unique_id = '$profile_uid' UNION SELECT company_name, profile_image, bg_image, description, business_uniqueid FROM business WHERE business_uniqueid = '$profile_uid' UNION SELECT ngo_name, profile_image, bg_image, ngo_desc, ngo_unique_id FROM ngo_resgitration WHERE ngo_unique_id = '$profile_uid'");
...
SELECT user_inf.pet_unique_id,addfriend.status FROM addfriend JOIN user_inf ON addfriend.parent_id = user_inf.pet_unique_id WHERE addfriend.child_id = '$parent_id'");
								...
								?>
                                                                <input type="hidden" name="id" value="<?= $profileinfo['pet_unique_id']; ?>">
								<a href="#" class="add_friend_bttn" id="profile-add-frnd">Add Friend</a>
								...
								if(!empty($key)) {
									...
									echo "<h3>Friend</h3>";
									echo "<button class='unfriend-btn' id=".$profileinfo['pet_unique_id'].">Unfriend</button>";
								...
								?>
                                <input type="hidden" name="id" value="<?= $profileinfo['pet_unique_id']; ?>">
								<!-- <a href="#" class="add_friend_bttn" id="profile-add-frnd">You</a> -->
                                                                <span class="friend_you">You</span>
                                                                
								
                                <?php  }
                                $cidqry=mysqli_query($conn,"SELECT * FROM conversation WHERE (user_one='$parent_id' AND user_two='$profile_uid') OR (user_one='$profile_uid' AND user_two='$parent_id')");
							$cid=mysqli_fetch_assoc($cidqry);
                               echo '<a href="message-list.php?user_id='.$profile_uid.'" class="msg_friend_bttn" id="msg-frnd">Message</a>';
								?>
...
<div id="about3disp" class="macy-top  masonry-0l">
<?php if(!empty($key)) {
	if($friend_status[$key]=='1') { ?> 
...
<div class="col-second col-second-right">
                        <div class="fr-list" id="friends-list">
                            ...
                            </span><p class="user-nm"><a href="friend-list-new.php?id=<?php echo $profile_uid; ?>">Friend List</a></p></h2>
                            <?php
...
$display2 = "SELECT user_inf.pet_name,user_inf.pet_unique_id,user_inf.profile_pic,addfriend.parent_id as sender,addfriend.child_id as recepient,addfriend.status FROM addfriend JOIN user_inf ON addfriend.child_id = user_inf.pet_unique_id WHERE addfriend.parent_id ='$profile_uid' and addfriend.status = '1' UNION SELECT user_inf.pet_name,user_inf.pet_unique_id,user_inf.profile_pic,addfriend.child_id as sender,addfriend.parent_id as recepient,addfriend.status FROM addfriend JOIN user_inf ON addfriend.parent_id = user_inf.pet_unique_id WHERE addfriend.child_id ='$profile_uid' and addfriend.status = '1' LIMIT 5";
...
<!--                <script type="text/javascript">
	...
	$(document).on('click','.msg_friend_bttn',function(e) {
...
beforeSend: function()
			...
			success: function(data)
		    {
	             ...
	             $('#uploading-post').hide();				
				$(data).insertAfter('#about3disp .stat-textarea');	
                 //$('#about3disp').append(data);				
