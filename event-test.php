...
<div class="main-content user-pro-dtls-page usr-feed-page-nw about-page usr-feed-page  groups-page create-mem-pages event-page event-join-page" id="about-load-ref">
<div class="main-content-inn col-three main-content-full">
<div class="col-first-side">
<?php require_once 'inc/event_profile_sidebar.php'; ?>
		...
		//	}
           ...
           // print_r($insert);die;
            ...
            //print_r($sqll);die;
              ...
              echo "<script>window.location.href='event-test.php?id=".$event_id."&name=".$e_name."'</script>";           
        }
...
?>		
<?php if($profileinfo1['user_id_fk']==$parent_id) { ?>		
...
<div class="usr-cnct-l-brw">
                                                <input type="file"  name="file" value="Uplaod Photo" /><span class="brw-t-btn">Change Picture </span></div>
...
<div class="pbl-b"><span><i class="fa fa-globe"></i> Published by</span> <a href="#"><?= $profileinfo1['event_by'] ?></a></div>
<div class="usr-pro-right">
<?php if($profileinfo1['user_id_fk']==$parent_id) { ?>
<div class="post-act-ins">
<button class="post-share-btn" id=""><i class="fa fa-user-plus"></i> Invite</button>
		
<a href="edit-event.php?id=<?php echo $event_id;?>" class="event-edit-button ev-edt-btn"><i class="fa fa-pencil-square-o"></i> Edit</a>
...
<div class="right">
<div class="post-share-btn"><a href="#"><i class="fa fa-user-plus"></i> Invite</a></div>
</div>
</div>
<div class="fri-cir-list">
    <?php                                                    
	
WHILE($eventusr=mysqli_fetch_assoc($grpqry1)) {
$grpqry=mysqli_query($conn,"SELECT pet_name, pet_unique_id, profile_pic
FROM user_inf
WHERE pet_unique_id =  '$eventusr[user_id_fk]'
UNION SELECT company_name, business_uniqueid, profile_image
FROM business
WHERE business_uniqueid =  '$eventusr[user_id_fk]'
UNION SELECT ngo_name, ngo_unique_id, profile_image
FROM ngo_resgitration
WHERE ngo_unique_id =  '$eventusr[user_id_fk]'");

					WHILE($grpres=mysqli_fetch_assoc($grpqry)) {
						?>
<div class="fri-cir-bx">
<div class="user-icn-img">
<span class="fr-img"><a href="about3.php?id=<?= $grpres['pet_unique_id'] ?>" class="frn_profile">
...
<span class="fr-nm"><a href="about3.php?id=<?= $grpres['pet_unique_id'] ?>" class="frn_profile"><?= $grpres['pet_name'] ?></a></span>
</div>
</div>
    <?php } } ?>	

</div>
<div class="fri-nc-g"><a href="about3.php?id=<?= $display1['pet_unique_id'] ?>"><?= $display1['pet_name'] ?></a> is going</div>
</div>
<div class="">
<div class="one-col-post">    
<div id="" class="macy-top masonry-0l masonry-post">
...
<input type="hidden" value="<?php echo $event_id;?>" name="event-id">
<input type="hidden" value="<?php echo $e_name;?>" name="event-name">
</div>
...
<div class="fr-list  rel-events" id="friends-list">
	<h2 class="fr-headnig">
