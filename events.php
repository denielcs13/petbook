...
//    $page_id=$_GET[id];
	...
	$image_tmpname=$_FILES['file']['tmp_name'];
        $target_path = $parent_id ."/Events/".$e_name."/profile_pic/" . basename($_FILES['file']['name']);
        $brk_path = $parent_id . "/Events/".$e_name."/profile_pic/";
		...
		//	}
           ...
           // print_r($insert);die;
            ...
            //print_r($sqll);die;
               ...
               //echo "<script>window.location.href='event-test.php?id=".$event."&name=".$e_name."'</script>";      
...
<div class="pro-timeline">
                       <?php if($profileinfo1['e_profile_pic']=="") {
							...
							<?php
$grp_name=mysqli_query($conn,"SELECT event_date FROM events WHERE event_id='$event_id' AND user_id_fk='$parent_id'");
...
<div class="usr-cnct-l"><div class="usr-cnct-l-brw"><input type="file"  name="file" value="Uplaod Photo" /><span class="brw-t-btn">Change Picture </span>
</div><button type="submit" name="file"><i class="fa fa-upload"></i>Upload</button></div>
</div>
  </form>
</div>
...
<div class="pbl-b"><span><i class="fa fa-globe"></i> Published by</span> <a href="#"><?= $display1['pet_name'] ?></a></div>
...
<!--<button class="post-message-btn" id=""><i class="fa fa-envelope"></i> Message Guests</button>-->
</div>
</div>
</div>
</div>
...
<span class="fr-img"><a href="about3.php?id=<?= $grpres['pet_unique_id'] ?>" class="frn_profile"><?php if($grpres['profile_pic']=="") {
