...
<?php 
				...
				?>
                               ...
                               <?php
				$profileqry=mysqli_query($conn,"SELECT * FROM pet_meet WHERE id='$_GET[id]'");
...
document.getElementById('fade').style.display='block',4000">Like</a>
</div>
</div>
</div>
...
</a></span><p class="user-nm"><a href="about3.php?id=<?= $psname['pet_unique_id'] ?>"><?= $psname['pet_name'] ?></a></p></h2>
<h3><?= $propost['title'] ?></h3>
...
<?php 
  $id = $_GET['id'];
  $emails = $_POST['emails_sender'];
  $querys = "select * from pet_meet where id = '$id'";
...
<li style='display:inline-block;margin: 0;padding: 0;'><a href='https://www.tumblr.com/blog/petbooq1'><img src='http://petbooq.com/images/social-icon6.png' style=''/></a></li>
  ...
  ?>
<form class="pet-info-frm" id="pet_mate" onsubmit="return validateForms()" method="post">
...
<div class="form-row">
<label class="lbl">Email</label> 
<div class="inpts"><input type="email" placeholder="Email" id="emails_sender" name="emails_sender" /></div>
...
function validateForms() {
    var x = document.getElementById("emails_sender").value;
    if (x == "") {
        text = "Enter email";
