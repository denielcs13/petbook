...
UNION 
SELECT id AS tid,child_post_id,title,id,image,time,'post' AS source FROM post WHERE share_with='$parent_id' AND notified='0'
UNION
SELECT gu.group_user_id AS tid,g.user_id_fk,g.group_name,gu.group_id_fk,g.gr_profile_pic,gu.time,'group_users' AS source FROM group_users gu,groups g WHERE gu.user_id_fk='$parent_id' AND gu.status='0' AND g.group_id=gu.group_id_fk AND gu.notified='0'
ORDER BY time DESC");
?>
...
$post_thumb=$pname['profile_pic'];
	...
	<a class="notification-links" href="show-post.php?pid=<?= $not['id'] ?>">
...
<?php } ?>
                    ...
                    <div class="seeAll"><a class="see-all" href="notifications_all.php">See All</a></div>
                </div>
            </li>
