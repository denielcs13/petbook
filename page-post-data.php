<?php
                       ...
                       WHILE($postres=mysqli_fetch_assoc($disprun))  {
							...
							<p class="user-nm"><a href="about3.php?id=<?= $pname_res['pet_unique_id'] ?>"><?= $pname_res['pet_name']  ?></a></p></h2>
...
<a href="aj_page.php?id=<?= $postres['p_update_id'] ?>&pid=<?= $page_id ?>&im=<?= urlencode($post_images[$i]) ?> .ajcontent" data-featherlight="ajax">
...
<li class="pb-image-box">
<a href="aj_page.php?id=<?= $postres['p_update_id'] ?>&pid=<?= $page_id ?>&vid=<?= urlencode($post_videos[$i]) ?> .ajcontent" data-featherlight="ajax">
<video controls>
