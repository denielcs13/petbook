...
<span class="frmalert" id="form-alert">Your Post is Blank.Please write something or attach media</span>
</div>
</form>
</div>
</div>
...
<div class="sug-mem-box">
<h4>Page Likes <a href="page-mem-list.php?id=<?php echo $page_id;?>" class="vw-al">View all</a></h4>

<?php
$folqry=mysqli_query($conn,"SELECT pet_name, about_data, pet_unique_id, profile_pic
...
$(document).ready(function() {
	...
	$("#profile-post-form").on('submit',(function(e)  {
           ...
           if($(this).find("input[name='title']").val()=='' && $(this).find("textarea[name='description']").val()=='' && $(this).find("input[name='files[]']").val()=='') {
						 $(this).find('#form-alert').show();
					$(this).find('.frmalert').fadeOut(2000);
						 return;
		   }
		$.ajax({
...
<!--share-->
  ...
  <div class="modal-dialog modal-sm" role="document">
    ...
    $friendqry = "SELECT user_inf.pet_name,user_inf.powner_name,user_inf.pet_unique_id,user_inf.profile_pic,addfriend.parent_id as sender,addfriend.child_id as recepient,addfriend.status FROM addfriend JOIN user_inf ON addfriend.child_id = user_inf.pet_unique_id WHERE addfriend.parent_id ='$parent_id' and addfriend.status = '1' UNION SELECT user_inf.pet_name,user_inf.powner_name,user_inf.pet_unique_id,user_inf.profile_pic,addfriend.child_id as sender,addfriend.parent_id as recepient,addfriend.status FROM addfriend JOIN user_inf ON addfriend.parent_id = user_inf.pet_unique_id WHERE addfriend.child_id ='$parent_id' and addfriend.status = '1'";
	    ...
	    if(isset($_SESSION['business_unique_id'])) {
		$friendqry="SELECT pet_name, about_data, pet_unique_id, profile_pic
FROM user_inf, page_users
WHERE user_inf.status =  '1'
AND user_inf.pet_unique_id = page_users.user_id_fk
AND page_users.page_id_fk =  '$page_id'
...
AND business.business_uniqueid = page_users.user_id_fk
AND page_users.page_id_fk = '$page_id'
AND page_users.status =  '1' AND page_users.user_id_fk!='$parent_id'
UNION SELECT ngo_name,ngo_desc,ngo_unique_id,profile_image 
FROM ngo_resgitration,page_users
WHERE ngo_resgitration.status =  '1'
AND ngo_resgitration.ngo_unique_id = page_users.user_id_fk
...
SELECT ngo_resgitration.ngo_unique_id,ngo_resgitration.ngo_name,ngo_resgitration.profile_image,'ngo' AS source FROM business_like JOIN ngo_resgitration on ngo_resgitration.ngo_unique_id=business_like.user_id WHERE group_id='$parent_id' AND business_like.status='1'";
	  ...
	  if(isset($_SESSION['ngo_unique_id'])) {
		  $friendqry="SELECT user_inf.pet_unique_id,user_inf.pet_name,user_inf.profile_pic,'user' AS source FROM ngo_like JOIN user_inf on user_inf.pet_unique_id=ngo_like.user_id WHERE ngo_id='$parent_id' AND ngo_like.status='1' UNION SELECT business.business_uniqueid,business.company_name,business.profile_image,'business' AS source FROM ngo_like JOIN business on business.business_uniqueid=ngo_like.user_id WHERE ngo_id='$parent_id' AND ngo_like.status='1' UNION 
SELECT ngo_resgitration.ngo_unique_id,ngo_resgitration.ngo_name,ngo_resgitration.profile_image,'ngo' AS source FROM ngo_like JOIN ngo_resgitration on ngo_resgitration.ngo_unique_id=ngo_like.user_id WHERE ngo_id='$parent_id' AND ngo_like.status='1'";
	  }
$friendrun=mysqli_query($conn,$friendqry);
WHILE($friendlist=mysqli_fetch_assoc($friendrun)) {
	?>
