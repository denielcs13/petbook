...
/* $chk_share=mysqli_query($conn,"SELECT * FROM shares WHERE post_id='$_POST[pid]' AND sharer_id='$parent_id' AND share_with_id='$name' AND type='post'");
	...
	$postqry=mysqli_query($conn,"SELECT * FROM updates WHERE update_id='$_POST[pid]'");
	$postdata=mysqli_fetch_assoc($postqry);
	
	$postshareqry="INSERT INTO post(child_post_id,title,posts,url,image,img_count,video,vid_count, type,share_with,time) VALUES ('$parent_id','$postdata[title]','$postdata[posts]','$postdata[url]','$postdata[image]','$postdata[img_count]','$postdata[video]','$postdata[vid_count]','shared','$name','$share_datetime')";
	
	if(mysqli_query($conn,$postshareqry)) {
		echo "Post Shared With ".$spet_name['pet_name']."&nbsp;&nbsp;&nbsp;";
	}
