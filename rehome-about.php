...
<?php 
				...
				?>
                               ...
                               <?php
				$profileqry=mysqli_query($conn,"SELECT * FROM pet_rehome WHERE id='$_GET[id]'");
				$profileinfo=mysqli_fetch_assoc($profileqry);				
				$userqry=mysqli_query($conn,"SELECT pet_name,pet_unique_id,profile_pic FROM user_inf WHERE pet_unique_id='$parent_id' UNION SELECT company_name,business_uniqueid,profile_image FROM business WHERE business_uniqueid='$parent_id' UNION SELECT ngo_name,ngo_unique_id,profile_image FROM ngo_resgitration WHERE ngo_unique_id='$parent_id'");
				$userdata=mysqli_fetch_assoc($userqry);
...
document.getElementById('fade').style.display='block',4000">Home</a>
</div>
...
<?php 
  $id = $_GET['id'];
  $emails = $_POST['sender_email'];
  $querys = "select * from pet_rehome where id = '$id'";
...
<form class="pet-info-frm" id="rehome-pet_details" onsubmit="return validateForm()" method="post">
...
<div class="inpts"><span><?php echo $row["species"]?></span></div>
</div>

<div class="form-row">
<label class="lbl">Pet Name</label> 
<div class="inpts"><span><?php echo $row["pet_name"]?></span></div>
</div>
...
<div class="form-row">
<label class="lbl">Email</label> 
<div class="inpts"><input type="email" placeholder="Email" id="sender_email" name="sender_email"/></div>
<p id="demo" style="color:red;margin-left: 15%;"></p>
</div>

<div class="form-row frm-sub-btn">
<div class="inpts"><input type="submit" value="Submit" /></div>
</div>
...
function validateForm() {
    var x = document.getElementById("sender_email").value;
    if (x == "") {
        text = "Enter email";
