...
$display = "SELECT * FROM pg_updates WHERE p_update_id='$_GET[pid]'";
...
<div class="col-first-side">
				...
				$par_name=mysqli_query($conn,"SELECT pet_name,pet_unique_id,profile_pic,'user_inf' as source FROM user_inf WHERE pet_unique_id='$postres[user_id_fk]' UNION SELECT company_name,business_uniqueid,profile_image,'business' as source FROM business WHERE business_uniqueid='$postres[user_id_fk]' UNION SELECT ngo_name,ngo_unique_id,profile_image,'ngo_resgitration' as source FROM ngo_resgitration WHERE ngo_unique_id='$postres[user_id_fk]'");
...
<div class="col-first">
  ...
  <div class="one-col-post" id="<?= $postres['p_update_id'] ?>">
		 ...
		 <h3><?= $postres['title'] ?></h3>
		 <p class="pst-text"><?= $postres['page_data'] ?><p>
...
<a href="aj_box.php?id=<?= $postres['p_update_id'] ?>&im=<?= urlencode($post_images[$i]) ?> .ajcontent" data-featherlight="ajax">
...
<li class="pb-image-box">
<a href="aj_box.php?id=<?= $postres['p_update_id'] ?>&vid=<?= urlencode($post_videos[$i]) ?> .ajcontent" data-featherlight="ajax">
<video controls>
...
<!--Social sharing code end here-->
											...
											$likeqry=mysqli_query($conn,"SELECT * FROM page_likes WHERE post_id='$postres[p_update_id]' AND liker_unique_id='$parent_id'");
												$likecount=mysqli_num_rows($likeqry);
													...
													if($likecount > 0) {	
													?>
					<button class="post-like-button liked" id="<?= $postres['p_update_id'] ?>"><span class='p-act-bt'>Unlike</span></button>
												<?php }
												else {
													?>
					<button class="post-like-button" id="<?= $postres['p_update_id'] ?>"> <span class='p-act-bt'>Like</span></button>
												<?php } ?>
													
													
                                                    <button class="post-comment-btn" id="<?= $postres['p_update_id'] ?>">
<img src='images/comment-img.png' alt='' /> <span class='p-act-bt'>Comment</span></button>
													
                                                    <button class="post-share-btn" id="<?= $postres['p_update_id'] ?>">
<img src='images/post-share-icon.png' alt='' /> <span class='p-act-bt'>Share</span></button>
								...
								$commentqry=mysqli_query($conn,"SELECT * FROM page_comments WHERE post_id='$postres[p_update_id]' LIMIT 6");
